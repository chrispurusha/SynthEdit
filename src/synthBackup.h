...
// Single-patch backup: capture whatever the connected device reports as its
// current state (Voyager's Panel Dump, or a Korg-style device's Current
// Program Dump — see synth_request_state_dump() in synthComms.c) and save
// the exact bytes off the wire to a file the user picks, verbatim, as a
// generic .syx SysEx capture. Deliberately opaque — this file has no idea
// what any of those bytes mean, same spirit as G2-Edit's bank backup
// treating patch content as an opaque blob rather than re-serializing it.
//
...
// handle_curr_prog_dump()) with the complete raw SysEx message exactly as
// received off the wire (including the F0/F7 framing), before any decoding.
// A no-op unless synth_backup_current_patch() armed a pending capture —
// every other incoming dump (e.g. the one connect_without_identity() already
// requests at startup) passes through untouched.
