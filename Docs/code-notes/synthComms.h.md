...
The longer comments from `synthComms.h`, moved here 2026-09-12 so the code reads cleanly. The code points at each as `// notes §k`. Verbatim and in file order; each is titled by what it documents.

## 1. `synth_decode_moog_name()`
...
## 2. `synth_moog_single_preset_dump_intact()`
...
## 7. `synth_send_korg_current_program_dump()`
...
## 8. `synth_send_korg_program_data_dump()`
...
## 9. `synth_apply_moog_panel_dump_locally()`
...
## 11. `synth_flush_pending_param_send()`
...
## 14. `synth_request_single_preset_dump()`
...
## 15. `synth_request_all_presets_dump()`
...
## 19. `synth_request_korg_program_dump()`
...
## 20. `synth_send_korg_program_write_request()`
...
## 22. `synth_effective_name_maxlen()`
